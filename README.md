...
| **Sprint 2, Draping Realism** | 🔶 | Collision velocity injection fix: body/self-collision resolvers update `predicted[i] += correction` (not `= positions[i]`) to cancel push-out velocity while preserving tangential sliding. Cloth settles (mean speed 0.22 m/s). Arch/umbrella shape persists — flat initial placement + isometric bending rest angle prevents natural draping. Fixes identified; not yet implemented. |

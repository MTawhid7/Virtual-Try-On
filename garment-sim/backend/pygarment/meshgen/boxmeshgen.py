"""
    ...
    VENDORED & MODIFIED: Removed igl, sim_config, and texture_utils
    imports. CGAL triangulation replaced with triangle library.
"""
...
class Panel:
    ...
    def plot(self, pts, title):
        """Debugging scatter plot — stubbed in vendored version."""
        pass
        ...
        if len_pvertices == 0:
            ...
            if n_found_indices == 1:  # get index
                ...
                raise PatternLoadingError(
                    f'{self.__class__.__name__}::Corner stitch vertex has been added more than once to panel vertices!')
    ...
    def gen_panel_mesh(self, mesh_resolution, plot=False, check=False): 
        """
        This function generates the vertices inside the panel using the
        vertices along the edges.

        MODIFIED: Uses triangle library instead of CGAL.
        The 'Y' flag prevents Steiner point insertion on boundary
        segments, preserving stitch vertex alignment.

        Input:
            * self (Panel object): Instance of Panel class
            * mesh_resolution (float): target edge length
            * plot (bool): unused (kept for API compatibility)
            * check (bool): unused (kept for API compatibility)
        """
        points = self.panel_vertices
        edge_verts_ids = tri_utils.get_edge_vert_ids(self.edges)

        new_vertices, f = tri_utils.gen_panel_mesh_triangle(
            points, edge_verts_ids, mesh_resolution
        )

        #Store — convert to list of numpy arrays to match expected format
        self.panel_vertices = [np.array(v) for v in new_vertices]
        self.panel_faces = [np.array(face) for face in f]
    ...
    def save_panel_mesh_obj(self, folder_path: Path):
        """
        Creates an obj file of the generated panel mesh.
        MODIFIED: Uses manual OBJ writer instead of igl.
        """
        ...
        with open(filepath, 'w') as fobj:
            for vi in v:
                fobj.write(f'v {vi[0]:.6f} {vi[1]:.6f} {vi[2]:.6f}\n')
            for fi in f:
                fobj.write(f'f {fi[0]+1} {fi[1]+1} {fi[2]+1}\n')
...
class BoxMesh(wrappers.VisPattern):
    ...
    def _get_edge_in_verts(self, edge, plot=False):
        ...
        if plot:
            pass  # Plotting stubbed in vendored version
        ...
        * Creates stitch segmentation
        ...
        for panelname in self.panelNames:
            ...
            for face in panel.panel_faces:
                face = np.asarray(face)  # ensure numpy for arithmetic ops
    ...
    def save_box_mesh_obj(self, with_normals=False, in_uv_config={}, mat_name='panels_texture'):
        """
        Creates an OBJ file of the generated box mesh.
        MODIFIED: Uses manual OBJ writer instead of texture_mesh_islands/save_obj.
        """
        if not self.loaded:
            print(f'{self.__class__.__name__}::{self.name}::WARNING::Pattern is not yet loaded. Nothing saved')
            return

        # Write a simple OBJ (no texture/materials — those are handled
        # downstream by garment-sim's glTF exporter)
        filepath = self.paths.g_box_mesh if hasattr(self, 'paths') else None
        if filepath is None:
            print(f'{self.__class__.__name__}::{self.name}::WARNING::No output path set. Nothing saved')
            return

        with open(filepath, 'w') as fobj:
            for vi in self.vertices:
                v = np.asarray(vi)
                fobj.write(f'v {v[0]:.6f} {v[1]:.6f} {v[2]:.6f}\n')

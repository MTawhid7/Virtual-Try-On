...
## 2026-03-07 (Update: Cusick Drape Deep Diagnostics & AL Multiplier Investigation)

### Current State

**Tier 4 — Stabilization Phase (In Progress / Still Unstable).**
In an attempt to resolve the `cusick_drape` explosion, we conducted a deep diagnostic investigation. We instrumented the solver to trace max velocity, kinetic energy, active contacts, max barrier gradient, and max Hessian across frames. We identified two major issues: a missing Hessian and contact normal computation for analytical colliders (Box/Cylinder), and the infinite accumulation of Augmented Lagrangian (AL) multipliers across frames for resting objects. We implemented fixes for both, but the visual simulation still performs poorly and the core instability has not improved.

### Progress

- **Deep Diagnostics Script:** Created `diagnose_cusick_deep.rs` logging solver metrics. Initialized with `init_with_material` (realistic cotton mass) to properly reproduce the visual explosion headless.
- **Physics Fix 1 (Analytical Collider Hessians):** Discovered that `CylinderCollider` and `BoxCollider` did not compute barrier Hessians or contact normals, resulting in `0.0` `Max_Hess` upon contact. Implemented manual distance gradient and Hessian evaluations for these inside `collision_pipeline.rs`.
- **Physics Fix 2 (AL Multiplier Clearance):** Diagnosed that `state.al_lambda` multipliers were never cleared across frames. Resting fabric under gravity accumulated upward penalty forces indefinitely until violent ejection. Added logic to reset `al_lambda` at the start of every timestep inside `pd_solver.rs`.

### Key Observations

- **Headless vs. Visual Discrepancy:** While the diagnostic script metrics indicated stability after fixes (Kinetic Energy dissipated safely), the Bevy visual simulation showed no improvement. The disparity suggests the visual setup has conflicting configurations or the physics integration loop operates differently under varying frame timings.
- **Hessian Preconditioning Failure:** We noticed that the Hessian proxy was previously disabled because it was mathematically incorrect (un-scaling the solution by the same magnitude). Without proper preconditioning, stiff barrier forces naturally destabilize the constant-system matrix of Projective Dynamics.

### Issues & Decisions

- **Issue:** Visual simulation remains explosive and fails to stabilize cloth on collision, despite headless traces showing numerical recovery.
- **Decision:** Do not write any more code for now. Stop aggressive incremental fixes. We must step back to reassess the foundational disparity between the diagnostic suite and the realtime visualizer.
- **Decision:** As instructed by the user, halted further coding to purely document current state and pause for architectural review.

### Next Steps

- [ ] Inspect the `vistio-viewer` initialization path and update loop to identify any parameter mismatches (timestep scaling, frame drop accumulators, material overrides) against the debug runner.
- [ ] Develop a visual debugging tool inside Bevy (rendering contact normals, barrier force vectors, violating vertices) rather than relying solely on abstract console logs.
- [ ] Reassess the IPC logic for the Augmented Lagrangian penalty strategy. Clearing multipliers prevents infinite accumulation but might cause continuous micro-bouncing by forcing the solver to resolve support forces from scratch every frame.

## 2026-03-08

### Current State

**Tier 4 — Numerical Robustness Achieved, but Performance & Convergence Stalled.**
The simulation no longer crashes with `NaN` explosions. We have successfully identified and patched the math vulnerabilities where `f32::INFINITY` was escaping `.is_nan()` checks and poisoning the SVD-based element projections. However, the `cusick_drape` scenario is currently **unusable for interactive visualization** due to extreme lag and a failure to achieve physical equilibrium (high residuals).

### Progress

- **NaN Explosion Resolution**:
  - **Root Cause Isolated**: Found that `f32::INFINITY` values generated by the linear solver (due to excessive Augmented Lagrangian penalties) propagated into `polar_decomposition_3x2`. Dot products of infinite vectors produced `NaN`, which then collapsed the entire mesh.
  - **Math Guards**: Implemented `!v.is_finite()` checks in `vistio-math` (`decomposition.rs` and `mat3x2.rs`) to intercept overflow before it mutates into `NaN`.
  - **Displacement Clamping**: Added a hard geometric bound (`1.0m` per iteration) in `pd_solver.rs`. This prevents the "infinity catapult" effect where a single stiff collision pushes a vertex to the edge of the floating-point universe in one sub-step.
- **Log Noise Reduction**: Removed verbose `eprintln!` calls from the inner IPC narrow-phase and gradient loops. This improved raw CPU throughput, but the simulation remains bottlenecked by solver convergence lag.
- **Warning Cleanup**: Resolved several compiler warnings regarding unused variables and unnecessary parentheses to maintain a clean build.

### Key Observations

- **Stiffness & Iteration Bottleneck**: The `cusick_drape` (cylinder contact) when combined with `self_collision` generates upwards of **150,000 active contacts**.
- **Convergence Failure**: The PD solver reaches its `max_iters (10)` every frame with a massive residual (`~96.0`). Because Projective Dynamics uses a *constant* system matrix $A$, it cannot "see" the barrier stiffness in the LHS. Resolving 150k constraints purely via RHS modifications is mathematically inefficient and leads to the extreme lag.
- **The "Unseen" Mesh**: The viewer appears empty/black. This indicates that while the mesh isn't "exploding" into `NaN`s anymore, it is still being stretched to extreme dimensions (residual=96) that likely teleport the vertices far beyond the camera's view frustum or clipping planes.

### Issues Encountered

- **CPU Scaling**: The sheer volume of self-contact pairs (400k+ BVH pairs, 150k narrow-phase contacts) is mathematically correct but computationally overwhelming for a single-threaded CPU.
- **AL Penalty Growth**: The Augmented Lagrangian `mu` parameter grows too fast for the PD solver to keep up, creating a "stiffness gap" that the current 10-iteration budget cannot bridge.

### Future Plan

- [ ] **Adaptive Substepping**: Implement sub-stepping specifically during high-stiffness contact to allow the barrier forces to resolve over multiple smaller time intervals.
- [ ] **Iteration Scaling**: Increase the PD solver iteration budget from 10 to 50+ to see if convergence is even possible for 150k contacts without LHS stiffness.
- [ ] **Tier 5 Acceleration**: Parallelize the broad-phase BVH and narrow-phase IPC on the GPU as it is now the clear primary bottleneck for standard drape scenarios.
